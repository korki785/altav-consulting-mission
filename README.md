...
Abonnement actuel : **gratuit**. Ce qui est réalisable dès maintenant, sans surcoût :
...
- **Starter** — 2 pipelines de transaction, soit exactement le split B2B / B2C. Meilleur rapport coût/bénéfice.
...
- Accès **Excel pré-inscrits** + **back-office Wix**.
- Décision : **qui porte le CRM en interne**.
- Fichier des certifiés de Stéphane → basculer les « Certification Promo 4 & 5 » payants de `CHAUD` vers `CLIENT`.
...
Valider ce séquencement → point de travail : **choix du CRM, migration + maquettes de séquences nurturing**.
